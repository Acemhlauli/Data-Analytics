...
This was achieved by integrating sentiment analysis and topic modelling to answer the central research question: **What is the overall emotional tone and thematic structure of this online discourse, and is the conversation constructive or polarized?**
...
│
├── notebooks/
│   └── Group\_3\_Mhlauli\_A\_&*Matlhare\_T*...ipynb
│
├── data/
│   └── .gitkeep (This folder is intentionally empty per the NDA)
|
│── Output/ Presentation slide.
|
├── README.md
└── LICENSE
...
1.  **Data Preprocessing:** The raw text was extensively cleaned. This included lowercasing, removing URLs, mentions, and hashtags, eliminating stopwords (using `nltk`), and performing lemmatization (using `spacy`) to normalize the text.
...
3.  **Topic Modeling:** **Latent Dirichlet Allocation (LDA)** (using `gensim`) was applied to the preprocessed corpus to identify and extract the dominant, hidden themes from the discourse.
...
* **3. Complete Absence of Polarization:** A significant finding was the **complete absence of polarization**. When mapping sentiment to topics, every single one of the seven themes registered a "$\rightarrow$ MIXED" sentiment profile. This confirms the discourse is stable and not fragmented into polarized blocks.
...
2.  **Review Analysis:** The full analytical process, from preprocessing to visualization, can be reviewed end-to-end in the `notebooks/Group_3_Mhlauli_A_&_Matlhare_T_...ipynb` file. All outputs, charts, and interpretations are saved within the notebook.
